...
/** envId 비트 수 (2비트, 최대값 3이지만 실제 범위는 0~2) */
export const ENV_ID_BITS = 2;

/** envId 최대값 (0=release, 1=stage, 2=dev) */
...
/** 디코딩된 EntityId 객체 (envId 포함) */
export interface DecodedEntityIdWithEnv {
  envId: number;
  pylonId: number;
  workspaceId: number;
  conversationId: number;
...
/**
 * envId 범위 검증 (0 ~ MAX_ENV_ID)
 */
function validateEnvId(envId: number): void {
  if (envId < 0 || envId > MAX_ENV_ID) {
    throw new RangeError(
      `envId must be between 0 and ${MAX_ENV_ID}, got ${envId}`
    );
  }
}
...
/**
 * envId를 포함한 EntityId 인코딩
 *
 * 비트 레이아웃 (23비트):
 * [envId: 2비트][pylonId: 4비트][workspaceId: 7비트][conversationId: 10비트]
 *
 * envId=0일 때 기존 encodeEntityId와 동일한 값 반환 (호환성)
 *
 * @param envId - 환경 ID (0=release, 1=stage, 2=dev)
 * @param pylonId - Pylon ID (1~10)
 * @param workspaceId - Workspace ID (1~127)
 * @param conversationId - Conversation ID (1~1023)
 * @returns 인코딩된 EntityId
 * @throws RangeError 범위를 벗어난 값이 입력된 경우
 */
export function encodeEntityIdWithEnv(
  envId: number,
  pylonId: number,
  workspaceId: number,
  conversationId: number
  ...
  const encoded =
    (envId << (PYLON_ID_BITS + WORKSPACE_ID_BITS + CONVERSATION_ID_BITS)) |
    (pylonId << (WORKSPACE_ID_BITS + CONVERSATION_ID_BITS)) |
    (workspaceId << CONVERSATION_ID_BITS) |
    conversationId;
...
/**
 * envId를 포함한 EntityId 디코딩
 *
 * @param id - 인코딩된 EntityId
 * @returns 디코딩된 envId, pylonId, workspaceId, conversationId 객체
 */
export function decodeEntityIdWithEnv(id: EntityId): DecodedEntityIdWithEnv {
  const conversationIdMask = (1 << CONVERSATION_ID_BITS) - 1; // 0x3FF (10비트)
  const workspaceIdMask = (1 << WORKSPACE_ID_BITS) - 1; // 0x7F (7비트)
  const pylonIdMask = (1 << PYLON_ID_BITS) - 1; // 0x0F (4비트)

  const conversationId = id & conversationIdMask;
  const workspaceId = (id >> CONVERSATION_ID_BITS) & workspaceIdMask;
  const pylonId = (id >> (WORKSPACE_ID_BITS + CONVERSATION_ID_BITS)) & pylonIdMask;
  const envId = id >> (PYLON_ID_BITS + WORKSPACE_ID_BITS + CONVERSATION_ID_BITS);
