 ...
 * BeaconClient와 유사한 구조로, link/unlink/list 요청을 처리한다.
 *
 * 프로토콜:
 ...
 * - link: 문서 연결 (성공, entityId 없음, 빈 path)
...
import type { LinkResult, SendFileResult } from '../../src/mcp/pylon-client.js';
...
class MockPylonServer {
  ...
  /**
   * 특정 entityId에 문서 미리 연결 (테스트 셋업용)
   */
  setLinkedDocs(entityId: number, docs: Array<{ path: string; addedAt: number }>): void {
    this._linkedDocs.set(entityId, docs);
  }
  ...
  private _handleRequest(request: {
    ...
    const { action, entityId, path, description } = request;

    // entityId 검증
    if (entityId === undefined || typeof entityId !== 'number') {
      return { success: false, error: 'entityId is required and must be a number' };
    }
    ...
    switch (action) {
      case 'link': {
        ...
        // entityId 유효성 검증 (간단히 0보다 큰 숫자인지 확인)
        if (entityId <= 0) {
          return { success: false, error: 'entityId not found' };
        }

        const docs = this._linkedDocs.get(entityId) || [];
...
describe('PylonClient', () => {
  ...
  // 테스트용 상수
  const TEST_ENTITY_ID = 132097; // encodeEntityId(1, 1, 1)
  ...
  beforeEach(() => {
    ...
    client = new PylonClient({ port: TEST_PORT, timeout: 3000 });
  });
  ...
  describe('constructor', () => {
    it('should_create_instance_with_default_options', () => {
      // Act
      const defaultClient = new PylonClient();

      // Assert
      expect(defaultClient).toBeInstanceOf(PylonClient);
      expect(defaultClient.port).toBe(9880);
      expect(defaultClient.timeout).toBe(5000);
    });

    it('should_create_instance_with_custom_port', () => {
      // Act
      const customClient = new PylonClient({ port: 9999 });
    ...
    it('should_create_instance_with_custom_timeout', () => {
      // Act
      const customClient = new PylonClient({ timeout: 10000 });
    ...
    it('should_create_instance_with_all_custom_options', () => {
      // Act
      const customClient = new PylonClient({ port: 8888, timeout: 15000 });
  ...
  describe('singleton', () => {
    it('should_return_same_instance_from_getInstance', () => {
      // Act
      const instance1 = PylonClient.getInstance();
      const instance2 = PylonClient.getInstance();
    ...
    it('should_not_throw_when_resetInstance_called_without_instance', () => {
      ...
      // Act & Assert
      expect(() => PylonClient.resetInstance()).not.toThrow();
    });
  });
  ...
  describe('link - happy path', () => {
    ...
    it('should_return_success_when_document_linked', async () => {
      ...
      // Act
      const result = await client.link(TEST_ENTITY_ID, path);
    ...
    it('should_link_multiple_documents_sequentially', async () => {
      // Arrange & Act
      await client.link(TEST_ENTITY_ID, 'docs/first.md');
      await client.link(TEST_ENTITY_ID, 'docs/second.md');
      const result = await client.link(TEST_ENTITY_ID, 'docs/third.md');
    ...
    it('should_return_docs_with_addedAt_timestamp', async () => {
      ...
      // Act
      const result = await client.link(TEST_ENTITY_ID, 'docs/spec.md');
  ...
  describe('link - error cases', () => {
    ...
    it('should_return_failure_when_entityId_not_found', async () => {
      // Arrange - 유효하지 않은 entityId (0 이하)
      const invalidEntityId = 0;

      // Act
      const result = await client.link(invalidEntityId, 'docs/spec.md');
    ...
    it('should_return_failure_when_path_is_empty', async () => {
      // Act
      const result = await client.link(TEST_ENTITY_ID, '');
    ...
    it('should_return_failure_when_linking_duplicate_document', async () => {
      // Arrange - 같은 문서 먼저 연결
      await client.link(TEST_ENTITY_ID, 'docs/spec.md');
  ...
  describe('unlink - happy path', () => {
    ...
    it('should_return_success_when_document_unlinked', async () => {
      // Act
      const result = await client.unlink(TEST_ENTITY_ID, 'docs/spec.md');
    ...
    it('should_return_remaining_docs_after_unlink', async () => {
      // Arrange - 추가 문서 연결
      await client.link(TEST_ENTITY_ID, 'docs/readme.md');
  ...
  describe('unlink - error cases', () => {
    ...
    it('should_return_failure_when_document_not_found', async () => {
      ...
      const result = await client.unlink(TEST_ENTITY_ID, 'docs/nonexistent.md');
    ...
    it('should_return_failure_when_entityId_not_found', async () => {
      ...
      // Act
      const result = await client.unlink(invalidEntityId, 'docs/spec.md');
    ...
    it('should_return_failure_when_path_is_empty', async () => {
      // Act
      const result = await client.unlink(TEST_ENTITY_ID, '');
  ...
  describe('list - happy path', () => {
    ...
    it('should_return_empty_list_when_no_documents_linked', async () => {
      // Act
      const result = await client.list(TEST_ENTITY_ID);
  ...
  describe('list - error cases', () => {
    ...
    it('should_return_failure_when_entityId_not_found', async () => {
      ...
      // Act
      const result = await client.list(invalidEntityId);
  ...
  describe('connection failure', () => {
    it('should_reject_link_when_connection_fails', async () => {
      // Arrange - 서버 없이 연결 시도
      const clientWithWrongPort = new PylonClient({ port: TEST_PORT + 1000, timeout: 1000 });

      // Act & Assert
      await expect(clientWithWrongPort.link(TEST_ENTITY_ID, 'docs/spec.md')).rejects.toThrow();
    });
    ...
    it('should_reject_unlink_when_connection_fails', async () => {
      ...
      // Act & Assert
      await expect(clientWithWrongPort.unlink(TEST_ENTITY_ID, 'docs/spec.md')).rejects.toThrow();
    });
    ...
    it('should_reject_list_when_connection_fails', async () => {
      ...
      // Act & Assert
      await expect(clientWithWrongPort.list(TEST_ENTITY_ID)).rejects.toThrow();
    });
  });
  ...
  describe('timeout', () => {
    ...
    it('should_reject_link_when_server_does_not_respond', async () => {
      ...
      const shortTimeoutClient = new PylonClient({ port: TEST_PORT, timeout: 100 });

      // Act & Assert
      await expect(shortTimeoutClient.link(TEST_ENTITY_ID, 'docs/spec.md')).rejects.toThrow(
        /timeout/i,
      );
    });
    ...
    it('should_reject_unlink_when_server_does_not_respond', async () => {
      ...
      // Act & Assert
      await expect(shortTimeoutClient.unlink(TEST_ENTITY_ID, 'docs/spec.md')).rejects.toThrow(
        /timeout/i,
      );
    });
    ...
    it('should_reject_list_when_server_does_not_respond', async () => {
      ...
      // Act & Assert
      await expect(shortTimeoutClient.list(TEST_ENTITY_ID)).rejects.toThrow(/timeout/i);
    });
    ...
    it('should_use_custom_timeout_value', async () => {
      ...
      const customTimeoutClient = new PylonClient({ port: TEST_PORT, timeout: 50 });

      // Act
      const startTime = Date.now();
      try {
        await customTimeoutClient.link(TEST_ENTITY_ID, 'docs/spec.md');
      } catch {
        // expected
      }
  ...
  describe('connection behavior', () => {
    ...
    it('should_create_new_connection_for_each_request', async () => {
      // Act - 여러 번 호출
      const result1 = await client.link(TEST_ENTITY_ID, 'docs/a.md');
      const result2 = await client.link(TEST_ENTITY_ID, 'docs/b.md');
      const result3 = await client.list(TEST_ENTITY_ID);
    ...
    it('should_handle_sequential_operations', async () => {
      ...
      await client.link(TEST_ENTITY_ID, 'docs/spec.md');
      await client.link(TEST_ENTITY_ID, 'docs/readme.md');
      await client.unlink(TEST_ENTITY_ID, 'docs/spec.md');
      const result = await client.list(TEST_ENTITY_ID);
    ...
    it('should_handle_concurrent_operations', async () => {
      ...
      const results = await Promise.all([
        client.link(TEST_ENTITY_ID, 'docs/a.md'),
        client.link(TEST_ENTITY_ID, 'docs/b.md'),
        client.link(TEST_ENTITY_ID, 'docs/c.md'),
      ]);
  ...
  describe('sendFile - happy path', () => {
    ...
    it('should_return_success_when_file_sent', async () => {
      ...
      // Act
      const result = await client.sendFile(TEST_ENTITY_ID, filePath);
    ...
    it('should_include_description_when_provided', async () => {
      ...
      // Act
      const result = await client.sendFile(TEST_ENTITY_ID, filePath, description);
  ...
  describe('sendFile - error cases', () => {
    ...
    it('should_return_failure_when_entityId_not_found', async () => {
      ...
      // Act
      const result = await client.sendFile(invalidEntityId, 'C:\\test\\file.txt');
    ...
    it('should_return_failure_when_path_is_empty', async () => {
      // Act
      const result = await client.sendFile(TEST_ENTITY_ID, '');
    ...
    it('should_return_failure_when_file_not_found', async () => {
      // Act
      const result = await client.sendFile(TEST_ENTITY_ID, 'C:\\nonexistent\\file.txt');
  ...
  describe('sendFile - connection failure', () => {
    it('should_reject_when_connection_fails', async () => {
      ...
      // Act & Assert
      await expect(clientWithWrongPort.sendFile(TEST_ENTITY_ID, 'C:\\test\\file.txt')).rejects.toThrow();
    });
  });
  ...
  describe('sendFile - timeout', () => {
    ...
    it('should_reject_when_server_does_not_respond', async () => {
      ...
      // Act & Assert
      await expect(shortTimeoutClient.sendFile(TEST_ENTITY_ID, 'C:\\test\\file.txt')).rejects.toThrow(
        /timeout/i,
      );
    });
  });
